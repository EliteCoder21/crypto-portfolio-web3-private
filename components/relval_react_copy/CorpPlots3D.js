...
let file_path = "https://gist.githubusercontent.com/Eesha-Jain/759b2125e756e0e9aa6e90b77105802d/raw/df76df364c256062c0a9b9dc7e3cf509ff6999f5/Corp_Data_imputed.csv"; //"./plotted_datasets/Corp_Data_imputed.csv";
...
function CorpPlots() {
  ...
  useEffect(() => {
    async function generate2D() {
      ...
      const csv_data = new CorpData(data, -1);

      // using the raw data, I created a new CorpData instance, which is just a class I created to make manipulation easier

      // Class CorpData() {
      //     MuniData(array d, int num)   constructs a CorpData instance
      ...
      const x_d = csv_data.get_col(["Risk"], true); // Risk, x
      const y_d = csv_data.get_col(["Yield"], true); // Yield, y
    ...
    async function generate3D() {
      ...
      let z_d = csv_data.get_col(["Price"], true); //Price, z
      ...
      let graphDiv = document.getElementById("graph2");
      let graphData = [
        {
          x: x_d,
          y: y_d,
  ...
  return (
    <div className="App">
      {/* <Plot
        ...
        layout={plotLayout2D}
      /> */}
      <Plot data={plotData3D} layout={plotLayout3D} />
    </div>
  );
}

export default CorpPlots;
