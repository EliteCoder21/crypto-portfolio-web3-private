export default class Utils {
    ...
    generateAccount() {
        ...
        if (!fs.existsSync(this.accountFile) || !current.hasOwnProperty("password") || content.length === 0) {
    ...
    fetchCoins() {
        if (!fs.existsSync(this.coinsFile) || fs.readFileSync(this.coinsFile).toString().trim() === "" || (Date.now() - 3600000) > fs.statSync(this.coinsFile).mtimeMs) {
            var pairs = [];
            var coins = JSON.parse(httpGet("https://api.coingecko.com/api/v3/coins/list"));
    ...
    /*fetchHistoricalData(id, currency, from, to) {
        ...
        if (!this.historicalDataExists(id, currency)) {
            const response = await fetch("https://api.coingecko.com/api/v3/coins/" + id + "/market_chart/range?vs_currency=" + currency + "&from=" + from + "&to=" + to);
    ...
    historicalDataExists(id, currency) {
        ...
        if (!fs.existsSync(historicalFile) || fs.readFileSync(historicalFile, "utf-8") === "" || (Date.now() / 1000) - refetchTime > fs.statSync(historicalFile).mtimeMs / 1000) {
            return false;
        }
        return true;
    }
        ...
        if (typeof directory === 'string' && directory.trim() !== '') {
            if (typeof is_dir === 'function' && is_dir(directory)) {
                ...
                for (var i = 0; i < files.length; i++) {
                    var file = files[i];
                    if (file !== "." && file !== "..") {
                        if (typeof filetype === 'function' && filetype(directory + "/" + file) === "dir") {
